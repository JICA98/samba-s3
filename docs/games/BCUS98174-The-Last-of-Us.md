...
12. *ARM64 Generic Timer Calibration in `busy_wait`:* Calibrated `rx::busy_wait` in `asm.hpp` from x86 TSC (3.5 GHz) to ARM64 generic timer (`cntvct_el0` at 19.2 MHz, 182x ratio). Eliminated 182x excessive spinning delay in SPU/RSX synchronization primitives. Accelerated level streaming / spore loading transition to 28.5–32.9 FPS (33.6 ms frametime).
13. *RSX Linux Priority Steering:* Implemented `setpriority(PRIO_PROCESS, 0, -8)` on Android inside `scoped_priority(+1)` in `Thread.cpp`. Grants the RSX rendering thread priority over background SPU workers to prevent Vulkan submission stalls.
14. *Hardware Barrier Diagnosis for Active Traversal:* Verified that active controllable traversal (Sarah walking in bedroom/hallway) demands 8 heavy threads (6 SPURS workers + 1 PPU main thread + 1 RSX rendering thread) running collision detection and cloth simulation. Because Snapdragon 8 Gen 3 has only 6 performance cores (1x Cortex-X4 + 5x Cortex-A720), the remaining threads contend for CPU resources, yielding 3.5–4.2 FPS (228–271 ms frametimes). Sustaining 30 FPS in active traversal on mobile requires next-gen 8-big-core mobile hardware or an engine binary patch that downsamples guest Havok cloth/physics compute from 60 Hz to 30 Hz.
