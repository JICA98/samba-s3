...
| **12** | Patch: `Unlock FPS` (`0x00036ab8 -> 0x38a00000`) | Remove guest engine 30 FPS sleep pacing | Replaces sleep pacing with a 0 us tight loop, causing PPU main thread to burn 96% CPU in `sys_timer_usleep` and starve SPU/RSX threads | **REJECTED (PPU Starvation)** |
...
| **15** | SPU Block Size `Mega` | Inline SPU basic blocks across unconditional branches | Breaks TLOU SPU deferred lighting and vertex shaders; yields black screen with 0 produced frames and triggers frame timeout | **REJECTED (Must remain Safe)** |
