...
- **Interactive Cutscene & Active Character Gameplay:** Transitioned into controllable character exploration in Sarah's bedroom with full real-time 3D rendering (mirrors reflecting Sarah rubbing eyes, bookshelf, lamp, soccer ball, posters) with active analog stick control validated.
- **3D Title Screen & Buffer Fixes:** Resolved black screen in 3D menu and bedroom by enabling `Video@@Read Color Buffers: true` and `Video@@Handle RSX Memory Tiling: true`. Fixed RSX SPU kick timeouts (`RsxKick: *** Timeout while waiting on RSX SPU kicks`) and SPU animation job timeouts (`ndlib/anim/anim-mgr.cpp:1762`) by allocating full `Core@@Max SPURS Threads: 6` and `Video@@Driver Wake-Up Delay: 1`.
