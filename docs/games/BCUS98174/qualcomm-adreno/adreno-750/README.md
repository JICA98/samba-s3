...
- **In-Game Rendering & Playable Character Control:** Rendered the entire opening prologue narrative sequence (Sarah on couch, Joel phone conversation, watch gift presentation, bedroom tuck-in) with high-fidelity models, dynamic lighting, and skin shaders at target **30.0 FPS** (32.3–33.7 ms frametime).
- **Interactive Cutscene & Active Character Gameplay:** Transitioned into controllable character exploration in Sarah's bedroom with full real-time 3D rendering (mirrors reflecting Sarah rubbing eyes, bookshelf, lamp, soccer ball, posters) with active analog stick control validated. Advanced through bedroom doorway into the upstairs hallway and reached Joel's bedroom door and the staircase landing.
- **3D Title Screen & Buffer Fixes:** Resolved black screen in 3D menu and bedroom by enabling `Video@@Read Color Buffers: true` and `Video@@Handle RSX Memory Tiling: true`. Fixed RSX SPU kick timeouts (`RsxKick: *** Timeout while waiting on RSX SPU kicks`) and SPU animation job timeouts (`ndlib/anim/anim-mgr.cpp:1762`) by allocating full `Core@@Max SPURS Threads: 6` and `Video@@Driver Wake-Up Delay: 1`. Capped `Core@@Max LLVM Compile Threads: 1` to prevent memory exhaustion during runtime compilation.
- **Thread Affinity Optimization:** Implemented ARM64 big.LITTLE core affinity pinning in `rpcs3/util/Thread.cpp`, `CPUThread.cpp`, and `RSXThread.cpp`. Restricted SPU, PPU, and RSX workers to Cortex-A720 and Cortex-X4 performance cores (cores 2–7), excluding Cortex-A520 efficiency cores (cores 0–1). This elevated active exploration framerates from 2.5–3.2 FPS up to 3.5–4.2 FPS (228–271 ms frametimes).
- **Telemetry Routing:** Disabled native in-emulator overlay (`Performance Overlay@@Enabled: false`) and routed real-time metrics through the in-app Compose UI overlay (`com.zenithblue.sambas3.DEBUG_MONITOR_SET --ez enabled true --es preset Performance`).
