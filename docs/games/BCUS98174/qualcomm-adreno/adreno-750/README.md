...
- **Interactive Cutscene & Active Character Gameplay:** Transitioned into controllable character exploration in Sarah's bedroom with full real-time 3D rendering (mirrors reflecting Sarah rubbing eyes, bookshelf, lamp, soccer ball, posters) with active analog stick control validated. Advanced through bedroom doorway into the upstairs hallway and foyer facing the staircase.
- **3D Title Screen & Buffer Fixes:** Resolved black screen in 3D menu and bedroom by enabling `Video@@Read Color Buffers: true` and `Video@@Handle RSX Memory Tiling: true`. Fixed RSX SPU kick timeouts (`RsxKick: *** Timeout while waiting on RSX SPU kicks`) and SPU animation job timeouts (`ndlib/anim/anim-mgr.cpp:1762`) by allocating full `Core@@Max SPURS Threads: 6` and `Video@@Driver Wake-Up Delay: 1`. Capped `Core@@Max LLVM Compile Threads: 2` to prevent memory exhaustion during runtime compilation.
- **Patch & Performance Analysis:** Narrative in-game scenes sustain **30.1 FPS** (33.4 ms frametime, APP CPU 332%, RSX 87%, PPU 78%). Active character traversal runs at 2.5–3.2 FPS due to 8-thread saturation (6 SPURS threads + RSX + PPU at 698% CPU) on Snapdragon 8 Gen 3's 6 performance cores. `Enable GPU Lighting` patch introduces square grid artifacts on v01.00; `Disable in-built MLAA` isolates cleanly without graphical regressions. Long input holds (>=600 ms) are required for reliable guest engine sampling during low-framerate traversal.
