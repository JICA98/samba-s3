...
Private artifacts are under `.git/gow3-13r-private/jobs/W27-RELEASE-PACKAGING-001/artifacts/`. The user explicitly authorized **“Test OnePlus now; Poco later.”** W29 installed the candidate ON release in place on the OnePlus, preserving app data. The Poco update remains deferred until it is reachable. Installed APK and core byte hashes match the candidate artifacts above; the runtime `S3CORE` message confirms that exact packaged library was loaded with `spu_shufb_tbl1=ON`.
...
## First integrated candidate run

W29 run 1 starts process `25158` with verified core `20a8d2abd`, option ON. Its actual SPU compilation log uses `s3cg4-mega-cortex-a34-base`; the `base` label has the previously described feature-label limitation and does not mean NEON is absent. The candidate rebuilt the 2,775-record SPU cache and reached visible gameplay. No controller inputs were sent by the assistant during this run; the user is playing while evidence is collected.

The user confirmed that the previously reported Kratos-disappearance bug **did not occur this time**. This is a non-reproduction, not proof of a fix. The snapshot of the live core log at about six minutes contains three RSX FIFO possible-desynchronization warnings, but no fatal renderer, access-violation or device-lost marker. Startup no-POS vertex warnings also occur in the old baseline and do not establish a later visibility defect.

SPU usage remains high. Linux process and per-thread `stat` counters are readable from ADB shell, although process maps are restricted. W29 records these counters every five seconds, preserving thread identities and timestamps; `CLK_TCK=100` is measured on the device. This permits scheduled CPU-time analysis without the intrusive SPU profiler. The current run uses an uncontrolled user route, reaches thermal SEVERE, and includes a two-minute diagnostic screen recording. It does **not** establish matched useful-frame FPS or a gameplay performance improvement. The full log/screenshot snapshot is under W29 `run1/phase3-check/`, named to reflect the user's correction that disappearance was not reproduced.
