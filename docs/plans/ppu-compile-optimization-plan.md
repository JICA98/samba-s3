...
The biggest wins are **concurrency/scheduling, avoiding unnecessary work, and preventing runtime compilation**, not adding more LLVM optimization passes. Upstream RPCS3 #18774 (July 1, 2026 — `jit_allocator` locking + 30→256 buckets + caller-thread recycling) is the most obvious missing optimization. Samba can additionally outperform desktop RPCS3 with Android big.LITTLE-aware worker scheduling, removal of unused ARM64 `PassBuilder`/`EarlyCSE` overhead, and aggressive executable manifest pre-discovery (RPCS3 issue #16883). Lower-priority experiments benchmark cheaper `CodeGenOptLevel`, `verifyModule`, and cache compression; tiered Interpreter→LLVM and function deduplication are R&D.
...
| `app/src/main/cpp/rpcsx/rpcs3/Emu/Cell/PPUTranslator.cpp/.h` + `SPULLVMRecompiler.cpp` | ARM64 `PassBuilder` compile-out validation (mirrors PPU path) |
...
- **Verify:** A/B on same device/title: wall-clock foreground 4 vs 8 workers, monitor `dumpsys cpuinfo`/thermal throttling, FPS after compile identical (correctness). No regression when background policy used.

**Step 1.3 — Remove unused ARM64 PassBuilder overhead (REVISED per review R1/R2)**
...
- In `SPULLVMRecompiler.cpp:2705-2733` the block is **unconditional**: 5 passes — `2728 EarlyCSEPass(true)`, `2729 SimplifyCFGPass`, `2730 DSEPass`, `2731 LICMPass` (`createFunctionToLoopPassAdaptor(LICMPass(LICMOptions()),true)`), `2732 ADCEPass` (exact via `grep -n addPass SPULLVMRecompiler.cpp:2728-2732`). Gate the **entire** `2705-2733` block (Loop/FAM/CGSCC/MAM/PassBuilder + all 5 `fpm.addPass` + `fpm.run` at ~2740) under `#ifdef ARCH_X64` (or `#if defined(ARCH_X64) && LLVM_AVAILABLE` consistent with PPU). This avoids per-module PassBuilder cost on ARM64 Android.
- ARM64 path should **not** construct any of the analysis managers at all when the passes are disabled (current upstream keeps all 5 disabled on ARM64 — EarlyCSE alone showed Naruto Storm 2 4:51→5:32, NFS MW 5:28→5:35 regression with unchanged size; the broader SPU pass set likewise not proven beneficial on ARM64).
- Do **not** enable any of these passes on ARM64 in this series — closed upstream PR justification.
- **Verify:** `grep -n "LoopAnalysisManager\|PassBuilder\|EarlyCSE\|SimplifyCFG\|DSEPass\|LICMPass\|ADCE" rpcsx/rpcs3/Emu/Cell/SPULLVMRecompiler.cpp` shows zero hits outside `#ifdef ARCH_X64` on ARM64; PPU `grep` unchanged. ARM64 build compiles; per-module timer `pass_manager_setup==0` on ARM64; x64 still runs all 5 SPU + 1 PPU passes; functional tests pass. Patch diff touches only `SPULLVMRecompiler.cpp`.
...
- Manifest persisted under `rpcs3::utils::get_cache_dir()` (`Emu/system_utils.cpp:125-130` wraps `fs::get_cache_dir()` → `<config_dir>/cache/`) + `"/ppu_manifest/<title_id>.json"` (use that helper, not hard-coded `cache/` or `dev_hdd0/game/<id>/` unless upstream convention requires migration). Keyed by: `game exe hash`, `firmware version/hash`, `RPCSX PPU cache ABI version (v7-kusa at PPUThread.cpp:5457)`, `llvm_cpu` (`JITLLVM.cpp:514/ cpu()`), `ppu relevant config` (`sysinfo`, `accurate_*`, `fixup` flags via `ppu_settings` enum at PPUThread.cpp:5410-5426), `patch state hash` (hash of `util/bin_patch.h:31` `patch_engine_version="1.2"` + `patchesList` JSON from `android/src/rpcsx-android.cpp:2715` `_rpcsx_patchEngineVersion` / `2717` `_rpcsx_patchesList`). See corrected line refs per R4/R7.
...
| `app/src/main/cpp/rpcsx/rpcs3/Emu/Cell/SPULLVMRecompiler.cpp:2705-2733` | **P1** Gate entire `LoopAnalysisManager/FAM/CGSCC/MAM/PassBuilder` + 5 passes (`2728 EarlyCSEPass(true)`, `2729 SimplifyCFGPass`, `2730 DSEPass`, `2731 LICMPass`, `2732 ADCEPass`) + `fpm.run` under `#ifdef ARCH_X64` — sole ARM64 PassBuilder compile-out edit | Fixes actual ARM64 wasted-work site (PPU already gated) |
...
- Matrix:
  ...
  - + ARM64 compile-out (verify no regression)
...
- [ ] `grep -n "LoopAnalysisManager\|PassBuilder\|EarlyCSE" PPUThread.cpp` shows whole block inside `#ifdef ARCH_X64` at 5840-5865 and ARM64 build has zero `pb.register*`/`fpm.addPass` calls (verified via `llvm` profile counter `pass_manager_setup==0` on arm64). `grep -n "LoopAnalysisManager\|PassBuilder\|EarlyCSE\|SimplifyCFG\|DSEPass\|LICMPass\|ADCE" rpcsx/rpcs3/Emu/Cell/SPULLVMRecompiler.cpp:2705-2733` shows entire manager + 5-pass block inside `#ifdef ARCH_X64` (SPULLVM is the only file needing edit).
...
| ARM64 PassBuilder guard already correct — prompt's wasted-work claim may be stale (current code already `#ifdef ARCH_X64`) | Low — no-op diff, wasted effort | Profiling in Step 0 proves whether ARM64 still pays cost (check SPULLVM path, or inline `PassBuilder` ctor cost). If already zero, mark N/A and close. |
...
3. ARM64 PassBuilder overhead — confirm PPU already `#ifdef ARCH_X64` at 5840-5865 is verification-only (no PPU file churn) and SPULLVM `2705-2733` unconditional 5-pass block is the sole ARM64 site to gate; ensure plan does **not** re-enable any pass on ARM64 (closed 4:51→5:32 regression).
4. Pre-discovery manifest scope vs RPCS3 #16883 — verify manifest path uses `rpcs3::utils::get_cache_dir()` (`system_utils.cpp:125-130` wraps `fs::get_cache_dir()`) and keys cover all invalidation dimensions (`v7-kusa` at PPUThread.cpp:5457, `llvm_cpu` at JITLLVM.cpp:514, `ppu_settings` at 5410-5426, `bin_patch.h:31` + `android/src/rpcsx-android.cpp:2715`/`2717` hash) and that on-demand PRX gap is acknowledged with tiered fallback planned.
